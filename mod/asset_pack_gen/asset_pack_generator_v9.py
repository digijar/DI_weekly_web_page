...
def gen_s7(presentation,content,country_flag,countries,color_hier,continent):
    ...
    for cty in content["region"]:
        for co in countries.keys():
            if co in cty:
                reg["countries"].append(co)
        for cont in continent.keys():
            if cont in cty:
                reg[cont] = continent[cont]
    reg["countries"] = list(set(tuple(reg["countries"])))
    print(reg)
    ...
    for r in reg.keys():
        ...
        if r == "countries":
            ...
            color_ind = list(reg.keys()).index(r) - 1
            chosen_color = color_hier[color_ind][0]+color_hier[color_ind][1]*256 + color_hier[color_ind][2]*256**2
            for ctys in reg[r]:
...
# def main(row_num):
    ...
    try:
        ...
        continent = {
            ...
            "Middle East":["Saudi Arabia","Armenia","Azerbaijan","Bahrain","Cyprus","United Arab Emirates","Georgia","Yemen",
                           "Iraq","Iran","Israel","Jordan","Kuwait","Lebanon","Oman","Qatar","Syria","Turkey"]
        }
        ...
        template_path = "../../../results/input/templates/AssetPackSample_v0.58.pptx"
        ...
        save_path = """../../../results/output/asset_packs/TestLLMSummarizing/{company_name}-AssetPack.pptx""".format(company_name=content["company_name"])
...
if __name__ == '__main__':
    import uvicorn
    uvicorn.run("asset_pack_generator_v9:app", host='127.0.0.1', port=5010, reload=True)