...
# def main(row_num):
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            response = await client.get(retrieve_data_URL+"/bq/"+str(row_num))
            info = response.json()
        ...
        content = {
            ...
            "biz_summary": info["col_summary"]["Business_Description"][2:len(info["col_summary"]["Business_Description"])-1],
            "deal_overview":info["col_summary"]["Deal_Intelligence_info"][2:len(info["col_summary"]["Deal_Intelligence_info"])-1],
            "sit_overview":[],
            "deal_stage":[],
            "deal_rationale":[],
        ...
        s13 = presentation.Slides(13) 
        image = [] 
        for s1 in range(1,len(s13.Shapes)+1): 
            if s13.Shapes(s1).Type == 13: 
                image.append(s1) 
        print(image,"\n---")

        pic_ind = 101 # change accordingly 
        s13.Shapes(pic_ind).Line.Visible = True 
        s13.Shapes(pic_ind).Line.ForeColor.RGB = 0 
        s13.Shapes(pic_ind).Line.Weight = 2 
        print("shape",pic_ind,"\n---")
...
if __name__ == '__main__':
    import uvicorn
    uvicorn.run("asset_pack_generator_v4:app", host='127.0.0.1', port=5010, reload=True)