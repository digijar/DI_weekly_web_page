        ...
        if check_result[rscol] != "nan":
            ...
            FROM
                ML.GENERATE_TEXT(
                    ...
                    (
                    SELECT
                        CONCAT('Summarize the following text in 45 words: ', `{tgt_column}`) AS prompt,
                        *
                    FROM
                        `testing-bigquery-vertexai.templates.Rolling_Shortlist`
                    WHERE Num = {num}
                    ...
                    STRUCT(
                    0.2 AS temperature,
                    90 AS max_output_tokens
                    )
                );
            """.format(tgt_column=rscol,num=row_num)
...
if __name__ == '__main__':
    import uvicorn
    uvicorn.run("retrieve_bigquery_service_v6:app", host='127.0.0.1', port=5011, reload=True)